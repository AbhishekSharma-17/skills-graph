...
# TODO: Set your upstream package URL
# For PyPI: "https://pypi.org/pypi/<package-name>/json"
# For npm: "https://registry.npmjs.org/<package-name>/latest"
# For other: set to None and implement custom logic in fetch_upstream_version()
UPSTREAM_VERSION_URL = None  # e.g., "https://pypi.org/pypi/agno/json"
...
def fetch_upstream_version():
    """Fetch the latest version from upstream. Customize for your source."""
    ...
    with open(SKILL_MD) as f:
        for line in f:
            if "references/" in line and ".md" in line:
                ...
                if start >= 0:
                    # Find end of path (before next backtick, pipe, space, or paren)
                    rest = line[start:]
                    end = len(rest)
                    for char in ["`", "|", " ", ")", "\n"]:
                        pos = rest.find(char)
