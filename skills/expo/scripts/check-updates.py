#!/usr/bin/env python3
"""Expo skill maintenance script.

Checks for updates to the Expo SDK and validates skill integrity.
"""
...
def check_npm_version() -> str | None:
    """Check latest version of expo on npm."""
    try:
        with urllib.request.urlopen(
            f"https://registry.npmjs.org/{PACKAGE_NAME}/latest", timeout=15
        ) as resp:
            data = json.load(resp)
...
def check_version(args):
    ...
    if latest:
        ...
        print("⚠️  Could not fetch latest version from npm")
        return 1
    return 0
...
def generate_report(args):
    ...
    print("=" * 60)
    print("Expo Skill — Status Report")
    print("=" * 60)
    print()
...
def main():
    parser = argparse.ArgumentParser(description="Expo skill maintenance")
    ...
    if len(sys.argv) > 1:
        ...
        if cmd == "--version":
            ...
            print("Sitemap check not implemented for npm packages")
            sys.exit(0)
        elif cmd == "--stale":
