#!/usr/bin/env python3
"""
GitHub Actions Skill Update Checker
=====================================
Checks for new upstream updates and validates skill integrity.

Usage:
    python scripts/check-updates.py              # Full report
    python scripts/check-updates.py --version    # Only check upstream changelog
...
UPSTREAM_VERSION_URL = "https://github.blog/changelog/label/actions/feed/"
    ...
    try:
        ...
        with urlopen(req, timeout=10) as resp:
            ...
            if "<pubDate>" in content:
                start = content.find("<pubDate>") + len("<pubDate>")
                end = content.find("</pubDate>", start)
                return {"latest_post": content[start:end].strip()}
            return {"latest_post": "Feed available — check manually"}
...
def check_version():
    ...
    print(f"  Tracked snapshot:  {tracked}")
    ...
    print(f"  Latest changelog:  {upstream.get('latest_post', 'unknown')}")
    print("\n  Note: GitHub Actions is a platform — check the changelog")
    print("  feed for new features that may need documentation updates.")
    return True
