#!/usr/bin/env python3
"""
Kubernetes Skill Update Checker
=================================
Checks for upstream Kubernetes version updates and validates skill integrity.

Usage:
    python scripts/check-updates.py              # Full report
    python scripts/check-updates.py --version    # Check latest K8s version
...
K8S_RELEASES_URL = "https://dl.k8s.io/release/stable.txt"
...
def check_version():
    """Compare tracked version against latest stable Kubernetes release."""
    ver = load_version()
    print(f"\n{'='*60}")
    ...
    try:
        req = Request(K8S_RELEASES_URL, headers={"User-Agent": "skill-checker/1.0"})
        with urlopen(req, timeout=10) as resp:
            latest = resp.read().decode().strip()
            print(f"  Latest stable:     {latest}")
            print(f"\n  Check https://kubernetes.io/releases/ for changes")
            return True
    except (URLError, KeyError) as e:
        print(f"\n  Could not check latest version: {e}")
        return None
...
def generate_report():
    print(f"\n{'#'*60}")
    print(f"  KUBERNETES SKILL — UPDATE REPORT")
