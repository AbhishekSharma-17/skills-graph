#!/usr/bin/env python3
"""
Playwright Skill Update Checker
=================================
Checks for upstream @playwright/test updates and validates skill integrity.
...
NPM_URL = f"https://registry.npmjs.org/{NPM_PACKAGE}/latest"
...
def check_version():
    ...
    try:
        req = Request(NPM_URL, headers={"User-Agent": "skill-checker/1.0"})
        with urlopen(req, timeout=10) as resp:
            data = json.loads(resp.read().decode())
            latest = data.get("version", "unknown")
            print(f"  Latest on npm:     {latest}")
            print(f"\n  Check https://github.com/microsoft/playwright/releases for changes")
            return True
    except (URLError, KeyError) as e:
        print(f"\n  Could not reach npm: {e}")
        return None
...
def generate_report():
    print(f"\n{'#'*60}")
    print(f"  PLAYWRIGHT SKILL — UPDATE REPORT")
