#!/usr/bin/env python3
"""
Redis Skill Update Checker
=================================
Checks for upstream Redis (Docker Hub) updates and validates skill integrity.

Usage:
    python scripts/check-updates.py              # Full report
    python scripts/check-updates.py --version    # Check Docker Hub version
...
DOCKER_IMAGE = "redis"
DOCKER_TAGS_URL = f"https://hub.docker.com/v2/repositories/library/{DOCKER_IMAGE}/tags?page_size=10&ordering=last_updated"
...
def check_version():
    """Compare tracked version against Docker Hub latest."""
    ver = load_version()
    print(f"\n{'='*60}")
    ...
    try:
        req = Request(DOCKER_TAGS_URL, headers={"User-Agent": "skill-checker/1.0"})
        with urlopen(req, timeout=10) as resp:
            data = json.loads(resp.read().decode())
            tags = [r["name"] for r in data.get("results", []) if r["name"] != "latest"]
            if tags:
                print(f"  Recent Docker tags: {', '.join(tags[:5])}")
            print(f"\n  Check https://redis.io/docs/latest/operate/oss_and_stack/stack-with-enterprise/release-notes/ for changes")
            return True
    except (URLError, KeyError) as e:
        print(f"\n  Could not reach Docker Hub: {e}")
        return None
...
def generate_report():
    print(f"\n{'#'*60}")
    print(f"  REDIS SKILL — UPDATE REPORT")
