#!/usr/bin/env python3
"""
Remotion Prompt Generator Update Checker
=========================================
Checks for upstream Remotion updates and validates skill integrity.

Usage:
    python scripts/check-updates.py              # Full report
    python scripts/check-updates.py --version    # Check Remotion npm version
...
def check_version():
    """Compare tracked Remotion version against npm latest."""
    ver = load_version()
    print(f"\n{'='*60}")
    print(f"  VERSION CHECK")
    print(f"{'='*60}")
    print(f"  Skill version:     {ver['skill_version']}")
    print(f"  Snapshot date:     {ver['content_snapshot_date']}")
    ...
    try:
        ...
        with urlopen(req, timeout=10) as resp:
            ...
            print(f"  Latest on npm:     {latest}")
            print(f"\n  Note: This skill tracks Remotion capabilities broadly.")
            print(f"  Check remotion.dev/docs for breaking changes.")
            return True
...
def generate_report():
    print(f"\n{'#'*60}")
    print(f"  REMOTION PROMPT GENERATOR — UPDATE REPORT")
