#!/usr/bin/env python3
"""
Tauri Skill Update Checker
===========================
Checks for new upstream Tauri releases and validates skill integrity.
...
UPSTREAM_VERSION_URL = "https://crates.io/api/v1/crates/tauri"
...
def fetch_upstream_version():
    try:
        ...
        with urlopen(req, timeout=10) as resp:
            ...
            return {"version": data.get("crate", {}).get("max_stable_version", "unknown")}
...
def check_version():
    ...
    latest = upstream.get("version", "unknown")
    print(f"  Latest upstream:   {latest}")

    if tracked.replace(".x", "") in latest:
        print("\n  Status: UP TO DATE")
    else:
        print(f"\n  Status: UPDATE AVAILABLE ({tracked} → {latest})")
    return True
