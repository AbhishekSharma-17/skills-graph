#!/usr/bin/env python3
"""
Trigger.dev Skill Update Checker
=================================
Checks for upstream Trigger.dev SDK updates and validates skill integrity.
...
NPM_PACKAGE = "@trigger.dev/sdk"
NPM_REGISTRY_URL = "https://registry.npmjs.org/@trigger.dev%2Fsdk/latest"
...
def check_version():
    """Compare tracked SDK version against npm latest."""
    ver = load_version()
    print(f"\n{'='*60}")
    print(f"  VERSION CHECK")
    print(f"{'='*60}")
    print(f"  Skill version:     {ver['skill_version']}")
    print(f"  Tracked SDK:       {ver['source_version_tracked']}")
    ...
    try:
        ...
        with urlopen(req, timeout=10) as resp:
            ...
            tracked = ver["source_version_tracked"]
            ...
            if latest == tracked:
                ...
                print(f"  Check https://trigger.dev/changelog for changes")
            return True
...
def generate_report():
    print(f"\n{'#'*60}")
    print(f"  TRIGGER.DEV SKILL — UPDATE REPORT")
