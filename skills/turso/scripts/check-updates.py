#!/usr/bin/env python3
"""
Turso Skill Update Checker
=================================
Checks for upstream Turso Database updates and validates skill integrity.

Usage:
    python scripts/check-updates.py              # Full report
    python scripts/check-updates.py --version    # Check GitHub releases
...
GITHUB_API_URL = "https://api.github.com/repos/tursodatabase/turso/releases/latest"
...
def check_version():
    """Compare tracked version against GitHub latest release."""
    ver = load_version()
    print(f"\n{'='*60}")
    ...
    try:
        req = Request(GITHUB_API_URL, headers={"User-Agent": "skill-checker/1.0"})
        with urlopen(req, timeout=10) as resp:
            data = json.loads(resp.read().decode())
            latest = data.get("tag_name", "unknown").lstrip("v")
            published = data.get("published_at", "unknown")[:10]
            print(f"  Latest release:    {latest} ({published})")
            if latest != ver["source_version_tracked"]:
                print(f"\n  UPDATE AVAILABLE: {ver['source_version_tracked']} -> {latest}")
                print(f"  Check https://docs.turso.tech/tursodb/changelog for changes")
            else:
                print(f"\n  Skill is up to date")
            return True
...
def generate_report():
    print(f"\n{'#'*60}")
    print(f"  TURSO SKILL — UPDATE REPORT")
