#!/usr/bin/env python3
"""
Zod Skill Update Checker
=========================
Checks for upstream Zod updates and validates skill integrity.
...
NPM_PACKAGE = "zod"
NPM_REGISTRY_URL = "https://registry.npmjs.org/zod/latest"
...
def check_version():
    ...
    print(f"  Tracked Zod:       {ver['source_version_tracked']}")
    ...
    try:
        ...
        with urlopen(req, timeout=10) as resp:
            ...
            if latest.startswith(tracked.replace(".x", "")):
                print(f"\n  UP TO DATE")
            else:
                print(f"\n  UPDATE AVAILABLE: {tracked} -> {latest}")
                print(f"  Check https://github.com/colinhacks/zod/releases for changes")
            return True
...
def generate_report():
    print(f"\n{'#'*60}")
    print(f"  ZOD SKILL — UPDATE REPORT")
